...
public abstract class ImageUtil {
    ...
    public static BufferedImage scaleAndCrop(BufferedImage original, int targetWidth, int targetHeight) {
        if (original.getWidth() == targetWidth && original.getHeight() == targetHeight) {
            ...
            // Scale!
            BufferedImage scaled = scaleImage(original, newWidth, newHeight);

            // Maybe return a cropped subimage
            if (scaled.getWidth() > targetWidth) {
                int x = (scaled.getWidth() - targetWidth) / 2;
                return scaled.getSubimage(x, 0, targetWidth, targetHeight);
            } else if (scaled.getHeight() > targetHeight) {
                int y = (scaled.getHeight() - targetHeight) / 2;
                return scaled.getSubimage(0, y, targetWidth, targetHeight);
            } else {
                return scaled;
            }
        }
