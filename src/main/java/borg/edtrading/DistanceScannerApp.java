...
public class DistanceScannerApp {
	...
	public static void main(String[] args) throws IOException {
		...
		if (!screenshotFiles.isEmpty()) {
			...
			//templates = Template.fromFolder(new File(System.getProperty("user.home"), "BodyScanner")); // FIXME on mac
	...
	private static void processScreenshotFile(File screenshotFile) throws IOException, ParseException {
		...
		for (TextLine tl : ocrResult.getTextLines()) {
			...
			String text = tl.toText().trim();
			if (text.contains("→") && tl.getxInScreenshot() >= 850) {
				String[] labelAndValue = text.split("\\→");
				if (labelAndValue.length == 2) {
					String label = labelAndValue[0].replace(":", "").trim();
					if (MiscUtil.levenshteinError(label, "DISTANCE") <= 0.25f) {
						String fixedValue = labelAndValue[1] //
								...
								.replace(" ", "") //
								.trim(); //
						if (fixedValue.matches("[\\d\\.]+\\.\\d{2}")) {
							int lastDotIndex = fixedValue.lastIndexOf(".");
							String val = fixedValue.substring(0, lastDotIndex).replace(".", "") + "." + fixedValue.substring(lastDotIndex + 1);
							BigDecimal bdScanned = new BigDecimal(val);
							Pattern p = Pattern.compile("Distance_.+_(\\d{6})_.+_01.png");
							Matcher m = p.matcher(screenshotFile.getName());
							if (m.find()) {
								BigDecimal bdEstimated = new BigDecimal(m.group(1));
								if (bdEstimated.subtract(bdScanned).abs().floatValue() < 30f) {
									success = true;
								} else {
									logger.error("OFF:  " + bdScanned + " <=> " + bdEstimated);
								}
							} else {
								logger.error("FILE: " + screenshotFile.getName());
							}
						} else {
							logger.error("PATT: " + fixedValue);
						}
					} else {
						logger.error("LEVS: " + label);
					}
				}
			}
		}
